...
def SimonGameSeq():
    ...
    def lose():
        if myGlobal == 0: #this prints a statement for all of the newbs out there
            GPIO.output(leds, True)
            sleep(0.5)
            GPIO.output(leds, False)
            sleep(0.5)
            print "Get R3kt newb. You couldn't even get past round one!"

        elif myGlobal == 1: #for those Grammar Nazis out therez
            GPIO.output(leds, True)
            sleep(0.5)
            GPIO.output(leds, False)
            sleep(0.5)
            print "You survived for {} round!".format(myGlobal)
    ...
    try:
        while(True):
            ...
            for s in seq: #controls the speed of the sequence depending on your current round
                if myGlobal < 5:
                    ...
                    GPIO.output(leds[s],False)#shuts the LEDs off
                    sounds[s].play()
                    sleep(0.6)
                    GPIO.output(leds[s], False)
                    sleep(0.15)
            ...
            while (count < len(seq)):
                ...
                count += 1
            myGlobal += 1 #Print this value at loss of game, and keeps track of current round
